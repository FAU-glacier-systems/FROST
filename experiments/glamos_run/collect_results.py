...
# Global paths
# RGI_FILES_PATH = "../../data/raw/glamos_run/Split_Files"
...
def load_and_collect_results():
    ...
    # Load RGI CSV
    if not os.path.exists(RGI_FILES_PATH):
        print(f"Error: CSV file {RGI_FILES_PATH} not found.")

    rgi_data = pd.read_csv(RGI_FILES_PATH)
    if rgi_data.empty:
        print(f"RGI_data empty")
...
def _merge_drop_duplicates(left: pd.DataFrame, right: pd.DataFrame, on: str,
...
def main():
    ...
    """
    combined_results = load_and_collect_results()
    ...
    if not combined_results.empty:
        ...
        if os.path.exists(INVERSION_PATH):
            ...
            print(
                f"Warning: INVERSION_PATH '{INVERSION_PATH}' not found. Skipping inversion merge.")
        ...
        if os.path.exists(SLA_PATH):
            ...
            sla_cols = [c for c in sla_df_all.columns if c in ("rgi_id", "sla",
                                                               "area_km2")]
            sla_df = sla_df_all.loc[
                :, sla_cols].copy() if sla_cols else pd.DataFrame(
                columns=["rgi_id", "sla", "area_km2"])
        else:
        ...
        if os.path.exists(GLAMOS_PATH):
            ...
            glamos_df = glamos_all.loc[
                :, glamos_cols].copy() if glamos_cols else pd.DataFrame(
                columns=["rgi_id"])
        else:
            print(
                f"Warning: GLAMOS_PATH '{GLAMOS_PATH}' not found. Skipping GLAMOS merge.")
            glamos_df = pd.DataFrame(columns=["rgi_id"])

        # Merge by RGI ID
        merged = combined_results.copy()
        if not inversion_df.empty:
            merged = _merge_drop_duplicates(merged, inversion_df, on="rgi_id",
                                            suffix="_inv")
        if not sla_df.empty:
            merged = _merge_drop_duplicates(merged, sla_df, on="rgi_id",
                                            suffix="_drop")
        if not glamos_df.empty:
            merged = _merge_drop_duplicates(merged, glamos_df, on="rgi_id",
                                            suffix="_drop")
