...
def plot_sla_vs_predictions(merged_df_sla: pd.DataFrame, out_path: Path) -> None:
    ...
    min = 2300
    max = 3601
    ax.plot([min - 100, max + 100], [min - 100, max + 100], "--", color=fg(), alpha=0.7,
            zorder=-4,
    ...
    ax.text(0.95, 0.05, txt,
            ...
            bbox=dict(facecolor=bg(), alpha=0.8),
    ...
    min = -2.
    max = 0.1
    ax.plot([min, max ], [min, max ], "--", color=fg(), alpha=0.7,
            zorder=-4,
            label="1:1 Correlation")
...
def main():
    ...
    plot_glamos_vs_predictions(merged_df_glamos, out_dir / "GLAMOS_regional_run")


    plot_sla_vs_predictions(merged_df_sla, out_dir / "SLA_regional_run")
