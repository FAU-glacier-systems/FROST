...
def main(rgi_id,SMB_model):
    ...
    # if reference data from GLAMOS for ELA information is available
    # the params_calibration.json/yams files is updated:
    # glacier-specific reference ELA and gradient values are added
    if str(SMB_model) == "ELA":
        CSV_FILE = "../../Data/GLAMOS/GLAMOS_analysis_results.csv"

        # If reference file with ELA measurements is available
...
if __name__ == '__main__':
    ...
    parser.add_argument("--SMB_model", type=str, help="SMB model selection (ELA, TI, ...).")
    args = parser.parse_args()
    main(args.rgi_id,args.SMB_model)
