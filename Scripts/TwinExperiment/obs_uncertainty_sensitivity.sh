...
if [ "$calibrate" = true ]; then
    ...
    for seed in "${seeds[@]}"; do
        ...
        for obs_uncertainty in "${obs_uncertainties[@]}"; do
            echo "Starting calibration with obs_uncertainty=$obs_uncertainty..."
            padded_obs_uncertainty=$(printf "%03d" "$obs_uncertainty")
            padded_seed=$(printf "%03d" "$seed")
            results_dir="Experiments/${rgi_id}/obs_uncertainty/${padded_obs_uncertainty}/Seed_${padded_seed}"
