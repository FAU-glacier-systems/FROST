...
def params(parser):
    ...
    parser.add_argument(
        ...
        help="Name of the input file for the climate outside the given datatime frame (time, delta_temp, prec_scali)",
    )
    parser.add_argument(
...
def initialize(params, state):
    ...
    with open(os.path.join('.', "params_smb_TI.json"), "r") as json_file:
        jsonString = json_file.read()

    oggm_mb_calib = json.loads(jsonString)

    params.temp_default_gradient = oggm_mb_calib[
        "smb_oggm_TI_temp_default_gradient"
    ]
    params.temp_bias = oggm_mb_calib["smb_oggm_TI_temp_bias"]
    params.prcp_fac = oggm_mb_calib["smb_oggm_TI_prcp_fac"]
    params.prcp_gradient = oggm_mb_calib["smb_oggm_TI_prcp_gradient"]

    # load climate data from netcdf file climate_historical.nc
    nc = Dataset(
        os.path.join('.', "climate_historical.nc")
    )
...
def update(params, state):
    #print("update climate at time : " + str(state.t.numpy()), str(params.yr_0), str(params.yr_1),  state.prec.shape[0])
    #print(np.shape(state.prec[0, :]))
    #print(params.clim_oggm_ref_period)
    ...
    if (state.t - state.tlast_clim_oggm) >= params.clim_update_freq:
        ...
        if (index_year >= 0) & (state.t <= params.yr_1) & (index_year < state.prec.shape[0]):
            #print('no correction')
            II = index_year
            delta_temp = 0.0
            prec_scal = 1.0
        ...
        # vertical temperature correction (lapse rates)
        ...
        # vertical precipitation correction (relative gradient)
        prcp_corr_multi = params.prcp_gradient * (state.usurf - params.ref_hgt)
        prcp_corr_multi = tf.expand_dims(prcp_corr_multi, axis=0)
        prcp_corr_multi = tf.tile(prcp_corr_multi, (state.temp.shape[1], 1, 1))
        #print('JOHANNES JOHANNES JOHANNES JOHANNES JOHANNES JOHANNES JOHANNES JOHANNES JOHANNES JOHANNES JOHANNES JOHANNES')
        #print('PRCP_CORR : ',params.prcp_gradient)
        #print('SHAPES : ',np.shape(params.prcp_gradient * (state.usurf - params.ref_hgt)),np.shape(state.precipitation),np.shape(prcp_corr_multi),np.shape(temp_corr_addi))
        #print('PRCP_CORR : ',prcp_corr_multi)

        # the final precipitation and temperature must have shape (12,ny,nx)
        state.air_temp = state.air_temp + temp_corr_addi
        state.precipitation = state.precipitation*(1.0+prcp_corr_multi)
